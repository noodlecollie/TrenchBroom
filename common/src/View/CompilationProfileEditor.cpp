...
QWidget* CompilationProfileEditor::createEditorPage(QWidget* parent) {
  ...
  upperLayout->setContentsMargins(
    LayoutConstants::MediumHMargin, LayoutConstants::WideVMargin, LayoutConstants::MediumHMargin,
  ...
  connect(
    m_taskList, &ControlListBox::itemSelectionChanged, this,
    &CompilationProfileEditor::taskSelectionChanged);
  connect(
    m_taskList, &CompilationTaskListBox::taskContextMenuRequested, this,
    [&](const QPoint& globalPos, Model::CompilationTask* task) {
      const int index = static_cast<int>(m_profile->indexOfTask(task));

      QMenu menu(this);
      QAction* moveUpAction = menu.addAction(tr("Move Up"), this, [=]() {
        moveTaskUp(index);
      });
      QAction* moveDownAction = menu.addAction(tr("Move Down"), this, [=]() {
        moveTaskDown(index);
      });
      menu.addSeparator();
  ...
  connect(
    m_removeTaskButton, &QAbstractButton::clicked, this,
    qOverload<>(&CompilationProfileEditor::removeTask));
  connect(
    m_moveTaskUpButton, &QAbstractButton::clicked, this,
    qOverload<>(&CompilationProfileEditor::moveTaskUp));
  connect(
    m_moveTaskDownButton, &QAbstractButton::clicked, this,
...
void CompilationProfileEditor::duplicateTask(const int index) {
  ...
  m_profile->insertTask(
    static_cast<size_t>(index) + 1, std::unique_ptr<Model::CompilationTask>(task->clone()));
  m_taskList->reloadTasks();
  m_taskList->setCurrentRow(index + 1);
  emit profileChanged();
}
